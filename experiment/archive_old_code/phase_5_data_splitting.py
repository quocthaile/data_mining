...
Default split strategy: **temporal** – rows are sorted by `last_activity_time`
  (fallback `first_activity_time`) and partitioned chronologically so that the
  model is never trained on future observations.  Group column defaults to
  `user_course_key` so no (user, course) pair straddles multiple splits.
...
- temporal: chronological split by time column (DEFAULT).
...
def merge_missing_columns_from_combined(
    ...
    with combined_csv.open("r", encoding="utf-8", newline="") as f:
        ...
        for idx, row in enumerate(reader, start=1):
            key = (row.get(key_column) or "").strip()
            if not key and key_column == "user_course_key":
                key = build_user_course_key(row)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="group",
        help="Split strategy (default: group — leakage-safe by user_course_key).",
    )
    parser.add_argument("--group-column", type=str, default="user_course_key",
                        help="Group column for group/hybrid splits (default: user_course_key).")
    ...
    try:
        ...
        if cfg.merge_combined:
            merged_columns = merge_missing_columns_from_combined(
                ...
                key_column=("user_course_key" if cfg.evaluation_unit == "user_course" else "user_id"),
        ...
        if cfg.time_column not in columns and cfg.time_fallback_column not in columns:
            raise RuntimeError(
                "Both time columns are missing in labeled data: "
                f"time_column='{cfg.time_column}', time_fallback_column='{cfg.time_fallback_column}'"
            )
