...
def make_metric_checks(
    ...
    add_check(
        split_name="valid",
        metric_name="pr_auc_ovr_macro",
        row=best_valid,
        threshold=0.60,
        note="PR-AUC macro captures ranking quality under imbalance",
    )
    add_check(
        split_name="test",
        metric_name="pr_auc_ovr_macro",
        row=best_test,
        threshold=0.60,
        note="PR-AUC macro on test should remain acceptable",
    )
    add_check(
        split_name="valid",
        metric_name="precision_low",
        row=best_valid,
        threshold=0.40,
        note="Precision of Low class controls intervention workload",
    )
    add_check(
        split_name="test",
        metric_name="precision_low",
        row=best_test,
        threshold=0.40,
        note="Precision of Low class on test for deployment realism",
    )
    add_check(
        split_name="valid",
