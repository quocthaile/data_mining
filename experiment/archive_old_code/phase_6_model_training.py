...
def main() -> int:
    ...
    try:
        ...
        if len(classes) < 2:
            only_class = classes[0] if classes else "Unknown"
            log(
                "WARNING: Training data has <2 classes. "
                f"Using fallback DummyClassifier with constant class='{only_class}'."
            )
            fallback_model_name = "dummy_single_class"
            fallback_estimator = DummyClassifier(strategy="most_frequent")
            fallback_estimator.fit(train_split.X, train_split.y)
            fallback_result = TrainResult(
            ...
            trained[fallback_model_name] = fallback_result
            ...
            for split in [valid_split, test_split]:
                ...
                comparison_row = {
                    ...
                    "cv_best_score": fallback_result.cv_best_score,
                    "cv_folds_used": fallback_result.cv_folds_used,
                    "train_seconds": fallback_result.train_seconds,
                    "best_params": json.dumps(fallback_result.best_params, ensure_ascii=True, sort_keys=True),
                }
