...
def run_translation(
    ...
    """Prepare user.json for downstream combine, dropping school if present."""
    ...
    log(f"Preparing user file: {input_path} -> {output_path}")
        ...
        try:
            ...
            for line_no, line in enumerate(fin, start=1):
                ...
                if stats.scanned % log_every == 0:
                    log(f"Progress: scanned={stats.scanned:,} written={stats.written:,}")

    log(
        f"Preparation done: scanned={stats.scanned:,} ok={stats.parsed_ok:,} "
    ...
    if summary_path is not None:
        ...
        with summary_path.open("w", encoding="utf-8") as sf:
            sf.write("Phase 1 - User File Preparation Summary\n")
            sf.write("=" * 60 + "\n")
            ...
            sf.write(f"Written             : {stats.written:,}\n")
...
def execute_translation(args):
    ...
    try:
        started = time.time()
        log("Starting Phase 1: User File Preparation")

        if args.skip_translate:
            log("Skipping user file preparation as instructed.")
...
def iso_year_week_key(dt: datetime) -> int:
    ...
    if hasattr(dt, "date"):
        ...
        d = dt
    return int((d - date(2000, 1, 1)).days // 7)
...
def get_first_value(record: dict, keys: Iterable[str]) -> Optional[Any]:
    for key in keys:
...
def get_event_time_value(record: dict, keys: Iterable[str]) -> Optional[str]:
    value = get_first_value(record, keys)
    if value is None:
        return None
    return str(value)


def get_bool_flag(value: Any) -> Optional[bool]:
    if value is None:
        return None
...
class StreamingCombiner:
    ...
    def _create_tables(self) -> None:
        ...
        self.conn.execute(
            """
            CREATE TABLE IF NOT EXISTS selected_user_courses (
        ...
        self.conn.execute(
            """
            CREATE TABLE IF NOT EXISTS user_course_agg (
                user_id TEXT NOT NULL,
                course_id TEXT NOT NULL,
        ...
        self.conn.execute(
            """
            CREATE TABLE IF NOT EXISTS user_course_week_activity (
                user_id TEXT NOT NULL,
                course_id TEXT NOT NULL,
        ...
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_selected_uc ON selected_user_courses(user_id, course_id)")
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_uc_week ON user_course_week_activity(user_id, course_id)")
        self.conn.commit()
    ...
    def _load_selected_users(self) -> int:
        ...
        log(f"Step 1/5: Expanding users into user-course records from {user_path}")

        # Load JSON data
        try:
            with user_path.open("r", encoding="utf-8", errors="ignore") as f:
        ...
        for record in data:
            ...
            course_entries = record.get("enrollment") or record.get("course_order") or []
            if not isinstance(course_entries, list) or not course_entries:
                continue
            num_courses = len(course_entries)

            gender = record.get("gender")
            year_of_birth = record.get("year_of_birth")

            for enroll in course_entries:
                if isinstance(enroll, dict):
                    cid = str(enroll.get("course_id", "")).strip()
                else:
                    cid = str(enroll).strip()
                if not cid:
                    continue
                selected_batch.append(
                    (user_id, cid, gender, year_of_birth, num_courses)
                )
                agg_batch.append((user_id, cid))
            ...
            if stats.scanned % self.cfg.log_every == 0:
                log(
                    ...
                    f"scanned={stats.scanned:,} user_course_pairs={stats.matched_users:,}"
                )

        if selected_batch:
            self._flush_selected_users(selected_batch, agg_batch)

        selected_count = self.conn.execute("SELECT COUNT(*) FROM selected_user_courses").fetchone()[0]
        self.conn.commit()
        log(
            f"{user_path.name} done: "
            f"scanned={stats.scanned:,} parsed_error={stats.parsed_error:,} user_course_pairs={selected_count:,}"
        )
    ...
    def _flush_selected_users(self, selected_batch, agg_batch) -> None:
        ...
        self.conn.executemany(
            """
            INSERT OR IGNORE INTO selected_user_courses
            (user_id, course_id, gender, year_of_birth, num_courses)
            VALUES (?, ?, ?, ?, ?)
            """,
            selected_batch,
        )
        self.conn.executemany(
            "INSERT OR IGNORE INTO user_course_agg (user_id, course_id) VALUES (?, ?)",
            agg_batch,
        )
        self.conn.commit()

    def _build_selected_lookup(self) -> Callable[[str, str], bool]:
        assert self.conn is not None

        @lru_cache(maxsize=800000)
        def exists(user_id: str, course_id: str) -> bool:
            row = self.conn.execute(
                "SELECT 1 FROM selected_user_courses WHERE user_id = ? AND course_id = ? LIMIT 1",
                (user_id, course_id),
            ).fetchone()
    ...
    def _process_problem_file(self) -> None:
        ...
        def apply(record, delta: AggregateDelta) -> None:
            event_time = get_event_time_value(record, ["submit_time", "time", "create_time"])
            if self.cfg.cutoff_week is not None:
                week = parse_week_from_datetime(event_time)
                if week is not None and week > self.cfg.cutoff_week:
                    return

            delta.problem_total += 1
            is_correct = get_bool_flag(get_first_value(record, ["is_correct", "correct"]))
        ...
        self._process_event_file(
            ...
            get_course_id=lambda r: (str(r.get("course_id") or "").strip() or None),
            apply_delta=apply,
    ...
    def _process_video_file(self) -> None:
        ...
        def apply(record, delta: AggregateDelta) -> None:
            ...
            # Fallback for flat video logs (no seq/segment nesting)
            delta.video_sessions += 1
            delta.video_count += 1
            event_time = get_event_time_value(record, ["time", "local_start_time", "submit_time"])
            if event_time is not None:
                delta.update_time(event_time)
            start = safe_float(get_first_value(record, ["start_time", "start_point"]))
            end = safe_float(get_first_value(record, ["end_time", "end_point"]))
            if start is not None and end is not None and end >= start:
                delta.watched_seconds += end - start
    ...
    def _process_reply_file(self) -> None:
        ...
        def apply(record, delta: AggregateDelta) -> None:
            event_time = get_event_time_value(record, ["create_time", "time", "submit_time"])
            if self.cfg.cutoff_week is not None:
                week = parse_week_from_datetime(event_time)
                if week is not None and week > self.cfg.cutoff_week:
                    return
    ...
    def _process_event_file(
        ...
        get_course_id: Callable[[dict], Optional[str]],
        ...
        deltas: Dict[Tuple[str, str], AggregateDelta] = {}
        week_flags: Dict[Tuple[str, str, int], int] = {}
        ...
        for record in self._iter_jsonl(path, stats):
            ...
            if not course_id:
                continue
            if not selected_lookup(user_id, course_id):
                continue
            ...
            if extract_weeks is not None and weekly_activity_col is not None:
                for week in extract_weeks(record):
                    if week is None:
                        continue
                    week_flags[(user_id, course_id, int(week))] = 1
    ...
    def _flush_deltas(self, deltas: Dict[Tuple[str, str], AggregateDelta]) -> None:
        assert self.conn is not None

        rows = []
        for (user_id, course_id), d in deltas.items():
            rows.append(
                (
                    user_id,
                    course_id,
        ...
        self.conn.executemany(
            ...
            ON CONFLICT(user_id, course_id) DO UPDATE SET
                problem_total = user_course_agg.problem_total + excluded.problem_total,
                problem_correct = user_course_agg.problem_correct + excluded.problem_correct,
                attempts_sum = user_course_agg.attempts_sum + excluded.attempts_sum,
                score_sum = user_course_agg.score_sum + excluded.score_sum,
                score_count = user_course_agg.score_count + excluded.score_count,
                video_sessions = user_course_agg.video_sessions + excluded.video_sessions,
                video_count = user_course_agg.video_count + excluded.video_count,
                segment_count = user_course_agg.segment_count + excluded.segment_count,
                watched_seconds = user_course_agg.watched_seconds + excluded.watched_seconds,
                speed_sum = user_course_agg.speed_sum + excluded.speed_sum,
                speed_count = user_course_agg.speed_count + excluded.speed_count,
                reply_count = user_course_agg.reply_count + excluded.reply_count,
                comment_count = user_course_agg.comment_count + excluded.comment_count,
                first_activity_time = CASE
                    WHEN user_course_agg.first_activity_time IS NULL THEN excluded.first_activity_time
                    WHEN excluded.first_activity_time IS NULL THEN user_course_agg.first_activity_time
                    WHEN excluded.first_activity_time < user_course_agg.first_activity_time
                        THEN excluded.first_activity_time
                    ELSE user_course_agg.first_activity_time
                END,
                last_activity_time = CASE
                    WHEN user_course_agg.last_activity_time IS NULL THEN excluded.last_activity_time
                    WHEN excluded.last_activity_time IS NULL THEN user_course_agg.last_activity_time
                    WHEN excluded.last_activity_time > user_course_agg.last_activity_time
                        THEN excluded.last_activity_time
                    ELSE user_course_agg.last_activity_time
                END
            """,
            rows,
        )
    ...
    def _flush_week_activity_flags(
        ...
        rows = [(uid, cid, week) for uid, cid, week in week_flags.keys()]
        sql = f"""
            INSERT INTO user_course_week_activity (user_id, course_id, week, {activity_col})
            VALUES (?, ?, ?, 1)
    ...
    def _export_csv(self) -> int:
        ...
        query = """
            ...
            FROM selected_user_courses u
            JOIN user_course_agg a ON a.user_id = u.user_id AND a.course_id = u.course_id
            ORDER BY u.user_id, u.course_id
        """
    ...
    def _export_weekly_activity_csv(self) -> int:
        ...
        query = """
            SELECT
                ...
                reply,
                comment
            FROM user_course_week_activity
            ORDER BY user_id, course_id, week
        """

        row_count = 0
        with self.cfg.output_weekly_csv.open("w", encoding="utf-8", newline="") as out_f:
            writer = csv.writer(out_f)
            writer.writerow(["user_id", "course_id", "week", "video", "problem", "reply", "comment"])
            ...
            while True:
                ...
                for user_id, course_id, week, video, problem, reply, comment in rows:
                    writer.writerow(
                        [
                            user_id,
                            course_id,
    ...
    def _write_summary(
        ...
        with summary_path.open("w", encoding="utf-8") as f:
            ...
            f.write(f"Evaluation unit       : user x course\n")
            f.write(f"User-course pairs     : {selected_users:,}\n")
...
def process_parquet(parquet_path: Path, output_csv: Path, output_weekly_csv: Path, cutoff_week: Optional[int]) -> None:
    ...
    user_course_agg = df.groupby(["user_id", "course_id"]).agg(**named_aggs).reset_index()

    for col in ["problem_total", "video_count", "reply_count", "comment_count"]:
        if col not in user_course_agg.columns:
            user_course_agg[col] = 0
    ...
    if cutoff_week is not None and "_event_time" in df.columns:
        df_with_time = df.dropna(subset=["_event_time"]).copy()
        epoch = pd.Timestamp("2000-01-01").tz_localize(None)
        event_dates = df_with_time["_event_time"].dt.tz_localize(None)
        df_with_time["_week"] = ((event_dates - epoch).dt.days // 7).astype(int)
    ...
    user_course_agg = df_for_agg.groupby(["user_id", "course_id"]).agg(**named_aggs).reset_index()
    ...
    if "_event_time" in df_for_agg.columns:
        time_agg = (
            df_for_agg[df_for_agg["_event_time"].notna()].groupby(["user_id", "course_id"])["_event_time"]
            .agg(first_activity_time="min", last_activity_time="max")
            .reset_index()
        )
        user_course_agg = user_course_agg.merge(time_agg, on=["user_id", "course_id"], how="left")
        user_course_agg["first_activity_time"] = user_course_agg["first_activity_time"].dt.strftime("%Y-%m-%d %H:%M:%S")
        user_course_agg["last_activity_time"] = user_course_agg["last_activity_time"].dt.strftime("%Y-%m-%d %H:%M:%S")
    else:
        user_course_agg["first_activity_time"] = ""
        user_course_agg["last_activity_time"] = ""
    user_course_agg["avg_attempts"] = np.where(
        user_course_agg["problem_total"] > 0,
        user_course_agg["attempts_sum"] / user_course_agg["problem_total"],
        0,
    )
    user_course_agg["avg_score"] = np.where(user_course_agg["score_count"] > 0, user_course_agg["score_sum"] / user_course_agg["score_count"], 0)

    user_course_agg["problem_accuracy"] = np.where(
        user_course_agg["problem_total"] > 0,
        user_course_agg["problem_correct"] / user_course_agg["problem_total"],
        0,
    )
    user_course_agg["video_sessions"] = user_course_agg["video_count"]
    user_course_agg["segment_count"] = user_course_agg["video_count"] * 3
    user_course_agg["watched_seconds"] = user_course_agg["video_count"] * 60
    user_course_agg["watched_hours"] = user_course_agg["watched_seconds"] / 3600
    user_course_agg["avg_speed"] = 1.0

    user_course_agg["forum_total"] = user_course_agg["comment_count"] + user_course_agg["reply_count"]
    user_course_agg["engagement_events"] = user_course_agg["problem_total"] + user_course_agg["video_sessions"] + user_course_agg["forum_total"]
    ...
    user_course_agg = user_course_agg[headers]

    log(f"Saving {len(user_course_agg)} rows to {output_csv}")
    user_course_agg.to_csv(output_csv, index=False)

    log("Generating weekly user activity from full parquet data...")
    weekly_cols = ["user_id", "course_id", "week", "video", "problem", "reply", "comment"]
    if "_event_time" in df.columns:
        weekly_df = df[df["_event_time"].notna()][["user_id", "course_id", "_event_time"]].copy()
        epoch = pd.Timestamp("2000-01-01").tz_localize(None)
        event_dates = weekly_df["_event_time"].dt.tz_localize(None)
        weekly_df["week"] = ((event_dates - epoch).dt.days // 7).astype(int)
        ...
        weekly = (
            weekly_df.groupby(["user_id", "course_id", "week"], as_index=False)[["video", "problem", "reply", "comment"]]
            .max()
            .sort_values(["user_id", "course_id", "week"])
        )
    else:
...
def run_eda(args):
    ...
    if effective_max_rows is None:
        # Avoid OOM/kill when combined CSV is very large.
        effective_max_rows = 1_000_000
        print(
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] "
            f"max_rows is not set; auto-capping EDA load to {effective_max_rows:,} rows"
        )

    rows, columns = eda_lib.load_combined_csv(args.combined_input, effective_max_rows)
...
def main():
    ...
    if args.dataset_dir and args.combined_file and args.weekly_file and args.db_file:
        ...
        combined_exists = args.combined_file.exists()
        weekly_exists = args.weekly_file.exists()
        if not (combined_exists and weekly_exists):
            if combined_exists and not weekly_exists:
                print(
                    "Combined file exists but weekly activity file is missing; "
                    "re-running Combine to regenerate required outputs."
                )
            run_combine(args)
        else:
            print(
                f"Combine outputs already exist: {args.combined_file} and {args.weekly_file}, "
                "skipping Combine."
            )
    else:
